...
function createApp(elements, options) {
  ...
  return (app = Vue.createApp({
    ...
    mounted() {
      ...
      options.query.mismatches = parseInt(sessionStorage.getItem("__mismatches") || "0", 10);
      ...
      function handleWrongServerId() {
        console.log("reloading because server ID mismatch");
        const mismatches = options.query.mismatches;
      ...
      const messageHandlers = {
        ...
        connect_error: (err) => {
          ...
          } else if (err.message == "Wrong server ID") {
            handleWrongServerId();
