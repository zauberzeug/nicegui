...
class Image(Group):
    ...
    def bind_source_to(self, target_object, target_name, forward=lambda x: x):
        bind_to(self.view, 'src', target_object, target_name, forward=forward)
        return self

    def bind_source_from(self, target_object, target_name, backward=lambda x: x):
        bind_from(self.view, 'src', target_object, target_name, backward=backward)
        return self

    def bind_source(self, target_object, target_name, forward=lambda x: x, backward=lambda x: x):
        bind_from(self.view, 'src', target_object, target_name, backward=backward)
        bind_to(self, 'src', target_object, target_name, forward=forward)
        return self
