...
class ElementFilter(Generic[T], Iterator[T]):
    ...
    def iterate(self, parent: Element, *, visited: List[Element] = []) -> Iterator[T]:
        for element in parent:
            ...
            if (self._types is None or isinstance(element, self._types)) and \
                ...
                (not self._contents or all(c in element_content for c in self._contents)) and \
    ...
    def within(self, *, element: Optional[Type] = None, marker: Optional[str] = None, instance: Union[Element, list[Element], None] = None) -> Self:
        if element is not None:
            assert issubclass(element, Element)
            self._within_types.append(element)
        if marker is not None:
            self._within_markers.append(marker)
        if instance is not None:
            self._within_elements.extend(instance if isinstance(instance, list) else [instance])
        return self

    def exclude(self, *, element: Optional[Element] = None, marker: Optional[str] = None, content: Optional[str] = None) -> Self:
        """Exclude elements with specific element type, marker or content."""

        if element is not None:
            assert issubclass(element, Element)
            self._exclude_types.append(element)
        if marker is not None:
    ...
    def not_within(self, *, element: Optional[Element] = None, marker: str = None, instance: Union[Element, list[Element], None] = None) -> Self:
        """Exclude elements which have a parent of a specific type or marker."""

        if element is not None:
            assert issubclass(element, Element)
            self._not_within_types.append(element)
        if marker is not None:
            self._not_within_markers.append(marker)
        if instance is not None:
            self._not_within_elements.extend(instance if isinstance(instance, list) else [instance])
        return self
