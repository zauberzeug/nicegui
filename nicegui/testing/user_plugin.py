...
async def user(nicegui_reset_globals,  # noqa: F811, pylint: disable=unused-argument
    ...
    async with core.app.router.lifespan_context(core.app):
        async with httpx.AsyncClient(app=httpx.ASGITransport(core.app), base_url='http://test') as client:
            yield User(client)
...
async def create_user(nicegui_reset_globals,  # noqa: F811, pylint: disable=unused-argument
    ...
    prepare_simulation(request)
    async with core.app.router.lifespan_context(core.app):
        yield lambda: User(httpx.AsyncClient(app=httpx.ASGITransport(core.app), base_url='http://test'))
    ui.navigate = Navigate()
    ui.notify = notify
