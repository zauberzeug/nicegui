...
async def test_sending_messages(create_user) -> None:
    ...
    userB = create_user()
    await userA.open('/')
    (await userA.type(text='Hello from screen A!', kind=ui.input)).trigger('keydown.enter')
    ...
    (await userB.type(text='Hello, from screen B!', kind=ui.input)).trigger('keydown.enter')
