...
# # Handle the case of tailwind outside NPM:
# USER_AGENT = 'Mozilla/5.0'
# request = urllib.request.Request('https://cdn.tailwindcss.com/3.2.4', headers={'User-Agent': USER_AGENT})
# with urllib.request.urlopen(request) as tailwindcss:
#     content = tailwindcss.read().decode()
#     match = re.search(r'{name:"tailwindcss",version:"(\d+\.\d+\.\d+)"', content)
#     if match:
#         version = match.group(1)
#         print(f"tailwind: {version} - https://cdn.tailwindcss.com")
#     with Path('nicegui', 'static', 'tailwind.min.js').open('w') as file:
#         file.write(content)
