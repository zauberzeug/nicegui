...
- **Prefer `contextlib.suppress` for ignoring exceptions**: Use `with contextlib.suppress(...):`
  instead of `try/except/pass`. Cleaner and more declarative. `contextlib` is from the standard library.
- **Use `ImportError` for optional dependencies**: Catch `ImportError` instead of `ModuleNotFoundError`.
  It's more defensive, catching both "not installed" and "installed but broken" cases.
