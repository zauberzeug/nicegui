...
- **Tailwind over `.style()`**: Use `text-(--mo-var)` and `bg-(--mo-var)` instead of `.style('color: var(--mo-var)')`. Only use `.style()` for things Tailwind can't express (gradients, box-shadows, `border-bottom`, non-standard border widths like `1.5px`).
...
- **CSS variables** are defined on `:root` (light) and `.body--dark` (dark) in `style.css`.
...
- **Scroll reveal**: Use the `mo-reveal` class. For staggered animations, use Tailwind's `delay-250!` / `delay-500!` (the `!` makes it `!important`, which overrides the `transition` shorthand in CSS that resets delay).
...
- **Quasar quirks**: Quasar sets its own `color: white` on `.q-header` and `background: #f8f8f8` on `<html>`. Override these explicitly when needed. The dark mode class is `.body--dark`, not Tailwind's `dark:`.
