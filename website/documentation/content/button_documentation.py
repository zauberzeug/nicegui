...
@doc.demo('Expandable Floating Action Button', '''
          ...
          is a button that can expand into multiple actions.
          While it is not a separate element in NiceGUI, it can be easily accessed with the generic `ui.element`.
          ''')
def fab() -> None:
    with ui.element('q-fab').props('icon="navigation" color="green"'):
        ...
        ui.element('q-fab-action').props('icon="sailing" color="green-5"') \
            .on('click', lambda: ui.notify('sailing'))
