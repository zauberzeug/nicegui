...
doc.text('Client-Side Secrets', '''
    ...
    **Where `client_id` is consumed: NiceGUI internals under `/_nicegui/`.**
    ...
    - There is no separate authentication layer on the per-client routes.
      Protecting the page that mints the `client_id` is what protects them.
...
doc.text('Examples Are Starting Points', '''
    ...
    - **Re-evaluate every threat-model assumption.**
      The `xterm` example, for instance, wires a browser to a Bash PTY by design.
      Its purpose is to demo the integration, not to be exposed on the open internet.
    - **Adapt the authentication example to your needs.**
      ...
      But production apps typically need rate limiting, CSRF protections for any state-changing endpoints you add,
      password hashing, and audit logging — none of which the example provides.
    ...
    - **Treat reload mode (`reload=True`) as developer-only.**
      Auto-reload watches the working directory and re-imports changed files.
      This is convenient locally, but not appropriate for production.
