...
def _example_card(example: Example) -> None:
    ...
    with ui.link(target=example.url).classes(
        'rounded-2xl overflow-hidden no-underline'
        ' transition-all duration-200 cursor-pointer'
        ' hover:-translate-y-0.5'
    ).style('background: var(--mo-surface); border: 1px solid var(--mo-border)'):
        with ui.element('div').classes('overflow-hidden aspect-video').style(
            'background: var(--mo-bg)'
        ):
            ui.interactive_image(example.screenshot).classes(
                'w-full h-full object-cover transition-transform duration-300'
            )
        with ui.column().classes('p-5'):
            ui.label(example.title).classes('text-lg font-semibold mb-1')
            ui.label(example.description).classes('text-sm leading-normal mb-3').style(
...
def create() -> None:
    ...
    with section('examples'):
        ...
        ui.link('Browse all examples \u2192', '/examples').classes(
            'mo-reveal mt-8 font-medium no-underline inline-flex items-center gap-1'
            ' hover:underline w-auto'
