...
def create() -> None:
    ...
    with section('why'):
        section_heading('', t('Why?'), center=True)

        with ui.column(align_items='center').classes('reveal gap-2 self-center text-center mt-6'):
            ui.label(t('\u201cWe like Streamlit but find it does too much magic when it comes to state handling.\u201d')) \
                .classes(f'italic {d.TEXT_19PX} leading-relaxed {d.TEXT_SECONDARY} max-w-[480px]')
            ui.link(t('Read the full story \u2192'), 'https://github.com/zauberzeug/nicegui/discussions/21') \
        ...
        with ui.row(wrap=False).classes('reveal gap-0 w-full mt-10 max-md:flex-col max-md:items-center'):
            _pillar('ph-file-py', t('Just Python'),
                    t('No HTML, CSS, or JavaScript needed. Build web interfaces entirely in Python '
                      'using familiar patterns and your existing tooling.'))
            ui.element().classes('w-px self-stretch bg-gray-500/15 max-md:hidden')
            _pillar('ph-trend-up', t('Grows with You'),
                    t('From a 10-line prototype to a multi-page production app \u2014 '
                      'same patterns, same codebase, no rewrite needed.'))
            ui.element().classes('w-px self-stretch bg-gray-500/15 max-md:hidden')
            _pillar('ph-battery-full', t('Batteries Included'),
                    t('100+ components, reactive data binding, charts & plots, 3D\u00a0scenes, native desktop apps, '
