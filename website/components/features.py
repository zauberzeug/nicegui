...
def _bento_card(icon: str, title: str, items: list[str]) -> None:
    ...
    with ui.column().classes(
        'mo-bento-card relative overflow-hidden rounded-2xl p-7 transition-all duration-200 cursor-default hover:-translate-y-0.5'
    ).style('background: var(--mo-surface); border: 1px solid var(--mo-border)'):
        ui.html(f'<i class="ph-duotone {icon}"></i>', sanitize=False) \
            .classes('text-[2rem] mb-4 text-(--mo-brand-blue)')
        ui.label(title).classes('text-xl font-semibold mb-3 tracking-tight')
        with ui.column().classes('gap-1.5'):
            for item in items:
                with ui.row().classes('items-start gap-3'):
                    ui.element('span').classes('w-[5px] h-[5px] rounded-full shrink-0 mt-[9px] opacity-50 bg-(--mo-brand-blue)')
                    ui.label(item).classes('text-[0.9375rem] leading-normal text-(--mo-text-secondary)')
