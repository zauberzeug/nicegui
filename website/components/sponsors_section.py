...
def create() -> None:
    ...
    with ui.element('section').classes('mo-sponsors-section').props('id=sponsors'):
        link_target('sponsors')
        with ui.element('div').classes('mo-sponsors-inner mo-reveal'):
            ui.html('<div class="mo-section-label">sponsors</div>', sanitize=False)
            ui.html('<h2 class="mo-section-title">Supported by the community.</h2>', sanitize=False)
