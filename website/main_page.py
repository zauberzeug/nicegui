...
def create() -> None:
    ...
    with ui.column().classes('dark-box p-8 lg:p-16 my-16'):
        with ui.column().classes('mx-auto items-center gap-y-8 gap-x-32 lg:flex-row'):
            ...
            ui.link(t('Documentation'), '/documentation').style('color: black !important') \
                    ...
                    with ui.row().classes('gap-8 justify-center'):
                        ...
                        for sponsor in SPONSORS['top']:
                            with ui.link(target=f'https://github.com/{sponsor}').classes('row items-center gap-2'):
                                ...
                                ui.label(f'@{sponsor}')
                    ui.markdown(t(
                        'as well as {normal} other [sponsors](https://github.com/sponsors/zauberzeug)'
                        ' and {contributors} [contributors](https://github.com/zauberzeug/nicegui/graphs/contributors).'
                    ).format(normal=SPONSORS['normal'], contributors=SPONSORS['contributors'])) \
                        .classes('bold-links arrow-links')
                else:
                    ui.markdown(t(
                        '{normal} [sponsors](https://github.com/sponsors/zauberzeug)'
                        ' and {contributors} [contributors](https://github.com/zauberzeug/nicegui/graphs/contributors).'
                    ).format(normal=SPONSORS['normal'], contributors=SPONSORS['contributors'])) \
                        .classes('bold-links arrow-links')
