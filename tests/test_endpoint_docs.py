...
def get_openapi_paths() -> Set[str]:
    return set(requests.get(f'http://localhost:{SeleniumScreen.PORT}/openapi.json', timeout=5).json()['paths'])


def test_endpoint_documentation_default(screen: SeleniumScreen):
    screen.open('/')
    assert get_openapi_paths() == set()


def test_endpoint_documentation_page_only(screen: SeleniumScreen):
    screen.ui_run_kwargs['endpoint_documentation'] = 'page'
    screen.open('/')
    assert get_openapi_paths() == {'/'}


def test_endpoint_documentation_internal_only(screen: SeleniumScreen):
...
def test_endpoint_documentation_all(screen: SeleniumScreen):
