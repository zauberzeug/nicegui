...
def test_ui_select_with_tuple_as_key(screen: Screen):
    class Model:
        selection: Optional[tuple[ui.option[str, tuple[int, int]]]] = None
    data = Model()
    options: list[ui.option[str, tuple[int, int]]] = [
        ui.option(label='option A', value=(2, 1)),
        ui.option(label='option B', value=(1, 2)),
    ]
    ...
    assert data.selection == (options[1],)
...
def test_ui_select_with_list_of_tuples(screen: Screen):
    class Model:
        selection: Optional[tuple[ui.option[str, tuple[int, int]], ...]] = None
    data = Model()
    options = [ui.option(label=f'{v[0]},{v[1]}', value=v) for v in ((1, 1), (2, 2), (3, 3))]
    data.selection = (options[0],)
...
def test_ui_select_with_list_of_lists(screen: Screen):
    class Model:
        selection: Optional[tuple[ui.option[str, list[int]], ...]] = None
    data = Model()
    options = [ui.option(label=f'{v[0]},{v[1]}', value=v) for v in ([1, 1], [2, 2], [3, 3])]
    data.selection = (options[0],)
