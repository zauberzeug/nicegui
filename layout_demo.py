...
button_looks = ButtonLayout().rounded().background.teal(0.9)
...
with ui.row().layout.add(shared).height.fixed.twenty().background.grey(0.4).align.main_axis.evenly().align.cross_axis.center().element:
    ui.button('12').layout.square().width.fixed.twelve().add(button_looks)
    ui.button('64').layout.width.fixed.sixty_four().add(button_looks).height.fractional.two_thirds()
    ui.button('1/6').layout.width.fractional.one_sixth().add(button_looks).on_hover(hover)
